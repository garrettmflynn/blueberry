...
export class Blueberry {
    constructor(ondata,onconnect,ondisconnect){
        if (ondata instanceof Function) this.ondata = ondata
        this.interface = new BlueberryWebBluetoothA();
        
        if (onconnect instanceof Function) this.interface.onConnectedCallback = onconnect
        if (ondisconnect instanceof Function) this.interface.onDisconnectedCallback = ondisconnect

        this.interface.onStateChange((data) => {
          if (this.ondata instanceof Function) this.ondata(data);
        })
    }
